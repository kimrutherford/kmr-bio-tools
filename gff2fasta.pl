...
my $outfile_upstream1000 = Bio::SeqIO->new( -format => 'fasta', -file => ">$ARGV[2].upstream1000.fasta" );
my $outfile_downstream1000 = Bio::SeqIO->new( -format => 'fasta', -file => ">$ARGV[2].downstream1000.fasta" );
...
while ( my $line = <GFF> ) {
    ...
    if ($type eq 'gene' || $type eq 'mt_gene' ) {
        ...
        my $output_upstream1000 = Bio::Seq->new(
            -seq        => $upstream_seq,
        ...
        if($array[6] eq '+') {
            ...
            $downstream_end=$gene_end+1000;
        }
        elsif ($array[6] eq '-') {
            $downstream_start=$gene_start-1000;
            $downstream_end=$gene_start-1;
        }
        my $downstream_seq = $db->seq( $array[0], $downstream_start, $downstream_end );
        my $output_downstream1000 = Bio::Seq->new(
            -seq        => $downstream_seq,
            -id         => $gene_name."_downstream1000",
            -display_id => $gene_name."_downstream1000",
            -alphabet   => 'dna',
        );
        ...
        if($array[6] eq '+') {
            ...
            $output_upstream1000 = $output_upstream1000->revcom();
            $output_downstream1000 = $output_downstream1000->revcom();
        }
        else {
	...
	if (length($upstream_seq) != 0) {
          $outfile_upstream1000->write_seq($output_upstream1000);
	}
	if (length($downstream_seq) != 0) {
          $outfile_downstream1000->write_seq($output_downstream1000);
	}
      }
